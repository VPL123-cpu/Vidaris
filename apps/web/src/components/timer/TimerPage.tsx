        ...
        {Array.from({ length: maxCycles }, (_, i) => (
          <motion.div
            ...
            className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
              i < cycles % maxCycles
                ? "bg-[#F5C044] shadow-[0_0_8px_rgba(245,192,68,0.7)]"
                : "bg-white/10"
            }`}
          />
        ))}
...
export function TimerPage() {
  ...
  // No local interval — TimerProvider in SaasLayout handles ticking globally

  const subject = getSubjectFromList(subjects, selectedSubjectId);
  const subjectColor = subject?.color ?? "#64748b";
  const subjectLabel = subject?.label ?? "Aucune matiere";
  ...
  // In hardcore mode: no reset during an active session
  const canReset = status !== "idle" && !(hardcoreMode && isRunning);

  return (
    <div className="space-y-6">
      {hardcoreMode && <HardcoreBanner />}
      ...
      <div className="relative flex flex-col items-center py-4">
        ...
        <svg width={280} height={280} viewBox="0 0 280 280" className="-rotate-90">
          ...
          <motion.circle
            ...
            style={{ filter: `drop-shadow(0 0 12px ${accentColor}80)` }}
          />
          <circle
            cx={140} cy={140} r={RADIUS - 18}
            fill="none"
            stroke={accentColor}
            strokeWidth={1}
            strokeOpacity={0.08}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
          <AnimatePresence mode="wait">
            {mode === "chrono" ? (
              ...
              >
                <Coffee size={22} className="text-[#2dd4bf]" />
      ...
      <div className="space-y-1.5">
        ...
        <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
          <motion.div
            animate={{
              width: `${Math.min((todayTotal / dailyGoalMinutes) * 100, 100)}%`,
            }}
            transition={{ duration: 0.6 }}
            className={`h-full rounded-full bg-gradient-to-r ${
              hardcoreMode
                ? "from-red-500 to-orange-400"
                : "from-[#F5C044] to-[#f7cb6a]"
            }`}
          />
        </div>
        {hardcoreMode && todayTotal < 120 && (
          <p className="text-[10px] text-red-400/70 text-right">
            Objectif quotidien requis en mode hardcore
          </p>
        )}
      </div>
      ...
      <div className="flex gap-3">
        ...
        >
          {isRunning ? (
            <>
              <Pause size={22} strokeWidth={2.5} /> Pause
            </>
          ) : (
            <>
      ...
      <div className="flex items-center justify-center gap-1.5 text-xs text-slate-600">
        <Zap size={11} className="text-[#F5C044]/50" />
        <span>2 XP par minute d&apos;etude</span>
      </div>
    </div>
  );
}
